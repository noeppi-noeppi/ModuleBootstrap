...
public class PathTests {
    ...
    void testUnionPath() throws IOException {
        try (FileSystem zipFS = FileSystems.newFileSystem(URI.create("jar:" + Paths.get("src", "test", "resources", "empty.zip").toUri()), Map.of())) {
            assertAll("Tests failed for zip file system, this should not happen, tests need to be adjusted", () -> runPathTests(zipFS));
        }

        FileSystem emptyFS = Path.of(URI.create("empty:/")).getFileSystem();
        runPathTests(emptyFS);
    }
    ...
    private static void testNameParts(FileSystem fs, Path path, String... names) {
        ...
        if (names.length > 0) {
            for (int i = 1; i <= names.length; i++) {
                ...
                assertEquals(i == names.length, fs.getPath(absStr, fromStart).startsWith(path));
                assertEquals(i == names.length && !path.isAbsolute(), fs.getPath("", fromEnd).endsWith(path));
